...
def extract_pdf(file_path: str) -> str:
    if not PdfReader:
        raise HTTPException(500, "PDF support not installed")
...
def extract_docx(file_path: str) -> str:
    if not docx:
        raise HTTPException(500, "DOCX support not installed")
...
async def upload_sermon(
    ...
    try:
        ...
        return {
            "success": True,
            "text": text[:8000],  # mobile-safe
            "uploaded_by": user.id  # 🔥 TRACK USER
        }
